        ...
        <div className="w-full max-w-full overflow-x-hidden bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            ...
            <div className="px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
                ...
                {viewMode === 'schedule' ? (
                    ...
                    {/* ===== Service Due — always-visible date-grouped section ===== */}
                    {(() => {
                        const serviceDueGroups = groupByServiceDue(allBookings);
                        if (serviceDueGroups.length === 0) return null;
                        return (
                            <div className="mt-6 bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                                <div className="bg-gradient-to-r from-[#1B365D] to-[#1B365D] px-4 sm:px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <Bell size={18} className="text-[#8CC63F]" />
                                        <div>
                                            <h3 className="text-lg sm:text-xl font-black text-white">Service Due</h3>
                                            <p className="text-white/70 text-xs">Customers scheduled for next service — nearest date first</p>
                                        </div>
                                        <span className="ml-auto bg-[#8CC63F] text-[#1B365D] text-xs font-black px-3 py-1 rounded-full">
                                            {allBookings.filter(b => b.customer?.next_service_date).length}
                                        </span>
                                    </div>
                                </div>
                                <div className="p-3 sm:p-6 space-y-6">
                                    {serviceDueGroups.map(([dateKey, bookings]) => {
                                        const todayStr = formatLocalYYYYMMDD(new Date());
